+++
title = "Software I like"
date = 2025-01-11

[taxonomies]
tags = ["software", "tools"]
+++

Tools and software I actually enjoy using. Yes, they exist.

<!-- more -->

## Terminal & Shell
- [zsh](https://www.zsh.org/)
- [Alacritty](https://alacritty.org/) - Terminal emulator
- [tmux](https://github.com/tmux/tmux) - Terminal multiplexer

## CLI Tools
- [Neovim](https://neovim.io/)
- [ripgrep](https://github.com/BurntSushi/ripgrep) - Fast grep replacement
- [bat](https://github.com/sharkdp/bat) - cat/less with syntax highlighting
- [eza](https://github.com/eza-community/eza) - Modern ls replacement
- [zoxide](https://github.com/ajeetdsouza/zoxide) - Smarter cd
- [bottom](https://github.com/ClementTsang/bottom) - System monitor
- [newsboat](https://newsboat.org/) - RSS reader

## Desktop
- Tiling window managers, specifically [Sway](https://swaywm.org/)
- [mpv](https://mpv.io/) - Media player

## Sync & Backup
- [Syncthing](https://syncthing.net/) - Continuous file synchronization
- [Unison](https://www.cis.upenn.edu/~bcpierce/unison/) - Bidirectional file sync

## Development
- [Rust](https://www.rust-lang.org/)
- [uv](https://github.com/astral-sh/uv) - Python package manager


---

## Want to check out
- [cargo-flamegraph](https://github.com/flamegraph-rs/flamegraph) - Profiler for Rust
- [Helix](https://helix-editor.com/) - Modal editor
- [Jai](https://inductive.no/jai/) - Programming language
- [Typst](https://typst.app/) - Modern typesetting system
- [Zig](https://ziglang.org/) - Programming language
